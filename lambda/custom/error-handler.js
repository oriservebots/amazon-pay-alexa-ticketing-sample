...
const errorMessages = require('config').AMAZON_PAY_ERRORS;
const utilities = require('utilities');
/**
	A detailed list of all payment declines and processing errorMessages can be found here:
	https://developer.integ.amazon.com/docs/amazon-pay/payment-declines-and-processing-errorMessages.html
**/


// These are errorMessages that will not be handled by Amazon Pay; Merchant must handle
function handleErrors(handlerInput) {
	...
	switch (actionResponseStatusMessage) {
		...
		// Permissions errorMessages - These must be resolved before a user can use Amazon Pay
		case 'ACCESS_DENIED':
		...
		// Integration errorMessages - These must be resolved before Amazon Pay can run
		...
		// Runtime errorMessages - These must be resolved before a charge action can occur
		...
		case 'ServiceUnavailable':
			errorMessage = errorMessages.errorMessage +
				errorMessages.errorMessagestatusCode + actionResponseStatusCode + '.' +
				errorMessages.errorMessagestatusMessage + actionResponseStatusMessage + '.' +
				errorMessages.errorPayloadMessage + actionResponsePayloadMessage;
			break;

		default:
			errorMessage = errorMessages.errorUnknown;
			break;
	}

	utilities.debug(JSON.stringify(handlerInput));
...
function handleBillingAgreementState(billingAgreementStatus, handlerInput) {
	...
	switch (billingAgreementStatus) {
		...
		case 'CLOSED':
		case 'SUSPENDED':
			errorMessage = errorMessages.errorBillingAgreement + billingAgreementStatus + errorMessages.errorBillingAgreementMessage;
			break;
		default:
			errorMessage = errorMessages.errorUnknown;
	}
...
function handleAuthorizationDeclines(authorizationStatusReasonCode, handlerInput) {
	...
	switch (authorizationStatusReasonCode) {
		...
		case 'TransactionTimedOut':
			errorMessage = errorMessages.authorizationDeclineMessage;
			break;
		default:
			errorMessage = errorMessages.errorUnknown;
	}
